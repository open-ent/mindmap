...
public class MindmapSearchingEvents implements SearchingEvents {

	private static final Logger log = LoggerFactory.getLogger(MindmapSearchingEvents.class);
	private SearchService searchService;

	public MindmapSearchingEvents(SearchService searchService) {
	...
	public void searchResource(List<String> appFilters, String userId, JsonArray groupIds, JsonArray searchWords, Integer page, Integer limit, final JsonArray columnsHeader,
		...
		if (appFilters.contains(MindmapSearchingEvents.class.getSimpleName())) {
			...
			searchService.search(userId, groupIds.getList(), returnFields, searchWords.getList(), page, limit, new Handler<Either<String, JsonArray>>() {
				...
				public void handle(Either<String, JsonArray> event) {
					...
					if (log.isDebugEnabled()) {
						log.debug("[MindmapSearchingEvents][searchResource] The resources searched by user are finded");
					}
				}
			});
		} else {
